...
if exists(config_filename):
    ...
    with open(config_filename) as json_file:
        config_filedata = load(json_file)
...
if not mkv_toolnix_path:
    mkv_merge_path = "mkvmerge"
    mkv_extract_path = "mkvextract"
    mkv_propedit_path = "mkvpropedit"
...
remove_unmatched_files = config["remove_unmatched_files"]

CRC_buffer = config["CRC_buffer"]

#checks if crucial files exist
if mkv_toolnix_path and not exists(mkv_merge_path):
...
def is_true(option_str):
    if option_str.lower() == "yes" or option_str.lower() == "y" or option_str.lower() == "true":
        return True
    return False

font_collector_log = is_true(font_collector_log)
skip_mux = is_true(skip_mux)
write_stats = is_true(write_stats)
remove_unmatched_files = is_true(remove_unmatched_files)

if not auto_font_q:
    print('\nWould you like to mux fonts automatically?')
    auto_font_q = input()
if is_true(auto_font_q):
...
for ep_num_absolute, ep_num in enumerate(ep_ranges):
    ...
    while i < len(options_data_temp):
        ...
        if "*" in options_data_temp[i] or "?" in options_data_temp[i]:
            #escape brackets for glob
            pattern = re.sub(r'[\[\]]', lambda m: '[[]' if m.group() == '[' else '[]]', options_data_temp[i])

            #retrieve all matching paths
            matches = glob.glob(pattern)

            if len(matches) > 1:
                print(f"Ambiguous path. {len(matches)} matching paths found:")
                print(matches)
                input('<press any key to exit>')
                quit()

            if len(matches) == 1:
                options_data_temp[i] = matches[0]
            
            if len(matches) == 0:
                print(f"\nNo matching paths found for:\n{options_data_temp[i]}")

                if remove_unmatched_files:
                    #find previous "(" (keep that bracket, so start deletion after it)
                    start_idx = None
                    for j in range(i - 1, -1, -1):
                        if options_data_temp[j] == ")":
                            start_idx = j + 1
                            break
                        if options_data_temp[j] == "--output":
                            start_idx = j + 2
                            break
                    
                    #find next ")" (delete including that bracket)
                    end_idx = None
                    for j in range(i + 1, len(options_data_temp)):
                        if options_data_temp[j] == ")":
                            end_idx = j
                            break
                    if end_idx is None:
                        #no closing bracket found: delete to end
                        end_idx = len(options_data_temp) - 1

                    if start_idx is not None and end_idx is not None:
                        #delete items from start_idx through end_idx (inclusive)
                        print(f"Removing:\n{options_data_temp[start_idx:end_idx + 1]}")
                        del options_data_temp[start_idx:end_idx + 1]
                        i = start_idx - 1
    ...
    if write_stats and exists(output_file):
        command = [
            "mkvpropedit",
            f"{output_file}",
        ...
        print("Writing track statistics and anonymizing.")
